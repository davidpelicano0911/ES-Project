...
interface LandingPageCardProps {
  ...
  onRemove?: (page: LandingPage) => void;
  isInCampaign?: boolean;
}

const LandingPageCard = ({ page, onDelete, onView, onRemove, isInCampaign = false }: LandingPageCardProps) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-lg flex flex-col justify-between min-h-[260px] transition-all">
      <div className="p-6 flex-1">
        <div className="flex justify-between items-start mb-3">
          <h3 className="text-lg font-semibold text-gray-900 leading-snug line-clamp-1">
            {replaceVariables(page.name || "Untitled Landing Page")}
          </h3>
        ...
        <p className="text-sm text-gray-600 mb-3 line-clamp-3">
          {replaceVariables(page.description || "No description available.")}
        </p>
      ...
      <div className="flex border-t border-gray-200 divide-x divide-gray-200">
        {!isInCampaign && hasRole("CONTENT_MARKETER") && (
          <button
            onClick={() => navigate(`/app/landing-pages/edit/${page.id}`)}
